...
function drawLevel1Lineup() {
  ...
  for (let i = 0; i < suspects1.length; i++) {
    ...
    imageMode(CENTER);
    if (suspectImgs1[i]) {
      image(suspectImgs1[i], x, y, imgW * scale, imgH * scale);
    } else {
      fill(190);
      noStroke();
      rectMode(CENTER);
...
function drawLevel1Inspect() {
  ...
  if (isHoldingMagnify1 && realFaces1[selected1]) {
    image(realFaces1[selected1], centerX, centerY, imgW, imgH);
  } else if (suspectFaces1[selected1]) {
    image(suspectFaces1[selected1], centerX, centerY, imgW, imgH);
  } else {
    fill(150);
    rectMode(CENTER);
