...
function drawLevel3Inspect() {
  ...
  if (isHoldingMagnify3 && realFaces3[selected3]) {
    image(realFaces3[selected3], imgX, imgY, imgW, imgH);
  } else if (suspectFaces3[selected3]) {
    image(suspectFaces3[selected3], imgX, imgY, imgW, imgH);
  } else {
    fill(190);
    noStroke();
    rectMode(CENTER);
...
function playLevel3Voice(index) {
  ...
  if (suspectVoices3[index]) {
    currentVoice3 = suspectVoices3[index];
    currentVoice3.play();
  }
}
